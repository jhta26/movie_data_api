...
class MovieAPIController {
    ...
    async getNowPlayingMovies(req, res, next) {
        var myInit = {
            method: 'GET',
            mode: 'cors',
            cache: 'default'
        ...
        let TMDBRequest = await fetch(
            `https://api.themoviedb.org/3/movie/now_playing?api_key=4d5bf8d10c6b03d87d1ad39ff4d7e296&language=en-US&region=US&page=1`
        );
        let TMDBResponse = await TMDBRequest.json();
        let results = await TMDBResponse.results;
        res.set({ 'access-content-allow-origin': ['*'] }).status(200).json(results);
